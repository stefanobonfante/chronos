    ...
    @Query(
            value =
                    "SELECT * FROM TB_STATO_MESI\n "
                            + "WHERE ANNO=? AND MESE=? AND COD_UTENTE=? AND CHIUSO=?", nativeQuery = true)
    List<CarichiEntity> queryStatoMese(int anno, int mese, String codiceUtente, String chiuso);

    @Query(
            value =
                    "select * from TB_CARICHI where ANNO=?1 and MESE=?2 and GIORNO=?3 and COD_UTENTE=?4 and COD_JOB=?5", nativeQuery = true
    )
